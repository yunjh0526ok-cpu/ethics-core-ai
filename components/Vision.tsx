...
const BusinessItem = ({ label, sub }: { label: string, sub: string }) => (
    <div className="flex items-center gap-3 p-3 border-b border-slate-700/50 hover:bg-white/5 transition-colors">
        ...
        <div>
            ...
            <div className="text-slate-400 text-xs group-hover:text-cyber-400 transition-colors">{sub}</div>
        </div>
    </div>
);
...
const Vision: React.FC = () => {
  return (
    <section id="vision" className="relative z-10 py-16 px-4 w-full max-w-[1800px] mx-auto scroll-mt-24 overflow-hidden">
      ...
      {/* Vision Cards (Keep standard width) */}
      ...
      {/* ================= SPLIT LAYOUT FOR ROBOT VISIBILITY ================= */}
      {/* 
          Strategy: 
          1. Use flex with a large gap (xl:gap-[380px]) to create a center stage for the robot.
          2. Restrict max-width of boxes to 450px to prevent overlapping.
          3. High transparency (bg-opacity/10 or /20) + backdrop-blur for glass effect.
      */}
      <div className="flex flex-col xl:flex-row items-center justify-center gap-12 xl:gap-[380px] w-full relative">
         
         {/* Left: Business Scope List */}
         <motion.div
         ...
         >
             ...
             {/* Box Transparency Adjusted: bg-slate-900/20 + backdrop-blur-sm */}
         ...
         {/* Right: Ethics Core Graphic Card */}
         <motion.div
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            // Height adjusted slightly to match left box visual weight
            // Transparency INCREASED: bg-[#0a0a12]/10
         ...
         >
            {/* Abstract Tech Graphic - made subtler */}
